...
export class TokenAnalysisService {
    ...
    async analyzeToken(token: TokenResult): Promise<TokenAnalysisResult> {
        ...
        const positionAnalysis = this.calculatePositionAnalysis(
            token,
            marketData
        );
    ...
    private calculatePositionAnalysis(
        token: TokenResult,
        ...
        const costBasisNative =
            token.balance.costBasisNative || currentValueNative;
    ...
    private filterMarketDataByAge(marketData: TokenPair[]): TokenPair[] {
        return marketData.map((pair) => {
            ...
            Object.entries(pair.priceChange || {}).forEach(
                ([period, value]) => {
                    if (isPeriodValid(period as TimePeriod)) {
