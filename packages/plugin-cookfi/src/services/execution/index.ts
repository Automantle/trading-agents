...
import MantleTradingService from "../trading/mantle";

export class ExecutionService {
    private isDryRun: boolean;
    private solanaTradingService: SolanaTradingService;
    private mantleTradingService: MantleTradingService;

    constructor(config: ExecutionServiceConfig = {}, runtime: IAgentRuntime) {
        this.isDryRun = config.isDryRun || false;
        this.solanaTradingService = new SolanaTradingService();
        this.mantleTradingService = new MantleTradingService(runtime);
    }
    ...
    private calculateBuyAmount(confidence: number): number {
        ...
        const confidenceScale =
            (confidence - MIN_LEVEL) / (MAX_LEVEL - MIN_LEVEL);
        const amount =
            MIN_BUY_AMOUNT +
            (MAX_BUY_AMOUNT - MIN_BUY_AMOUNT) * confidenceScale;
    ...
    private getTradingService(chain: string) {
        switch (chain.toLowerCase()) {
            ...
            case "mantle":
                return this.mantleTradingService;
            default:
    ...
    async executeDecision(
        ...
        if (this.isDryRun) {
            elizaLogger.log(
                `[DRY RUN] Would execute ${decision.recommendation} for ${token.symbol}`,
                {
        ...
        try {
            ...
            switch (decision.recommendation) {
                case "BUY": {
                    ...
                    elizaLogger.log(
                        `Executed BUY for ${token.symbol} (${amount} SOL)`
                    );
                    return {
                        success: true,
                        action: "BUY",
            ...
            elizaLogger.error(
                `Failed to execute ${decision.recommendation} for ${token.symbol}:`,
                error
            );
            return {
                success: false,
